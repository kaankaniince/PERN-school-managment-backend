...
const addTeacher = async ({fname, lname, email, password, lesson, role_id}) => {
    return sql("INSERT INTO teacher (fname, lname, email, password, lesson, role_id) VALUES ($1, $2, $3, $4, $5, $6)", [fname, lname, email, password, lesson, role_id]);
}
...
const addClassAssignments = async ({teacher_id, class_id}) => {
    return sql("INSERT INTO class_assignments (teacher_id, class_id) VALUES ($1, $2)", [teacher_id, class_id]);
}

const deleteTeacher = async (id) => {
    return sql("DELETE FROM teacher WHERE id = $1", [id])
}
...
const deleteClassAssignments = async (id) => {
    return sql("DELETE FROM class_assignments WHERE id = $1", [id])
}

const updateTeacher = async ({fname, lname, email, password, lesson, role_id, id}) => {
    return sql("UPDATE teacher SET fname = $1, lname = $2, email = $3, password = $4, lesson = $5, role_id = $6 WHERE id = $7", [fname, lname, email, password, lesson, role_id, parseInt(id)]);
}
...
const updateClassAssignments = async ({teacher_id, class_id, id}) => {
    return sql("UPDATE class_assignments SET teacher_id = $1, class_id = $2 WHERE id = $3", [teacher_id, class_id, parseInt(id)]);
}
